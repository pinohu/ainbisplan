...
const GENERATOR = 'ainbis-business-authority-plan-generator@0.1.0';
...
function parseArgs(argv) {
  const args = {
    ...
    mode: 'scaffold',
    sourceRepository: 'pinohu/ainbis'
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--inventory') {
      ...
      index += 1;
    } else if (token === '--blueprints-root') {
      args.blueprintsRoot = value;
      index += 1;
      ...
      args.mode = value;
      index += 1;
    } else if (token === '--source-repository') {
      args.sourceRepository = value;
      index += 1;
  ...
  if (!['scaffold', 'refresh', 'validate'].includes(args.mode)) {
    throw new Error('--mode must be scaffold, refresh, or validate');
  }
...
function printHelp() {
  console.log(`
Generate or validate one individualized AINBIS authority-plan scaffold per inventory business.
...
Modes:
  scaffold  Create missing plans and preserve existing reviewed plans.
  refresh   Refresh inventory and blueprint provenance; mark changed plans stale.
  validate  Verify 1:1 inventory-to-plan parity without writing.
`);
}
...
function validateInventory(inventory) {
  if (!inventory || !Array.isArray(inventory.businesses)) {
    throw new Error('Inventory must contain a businesses array');
  }
  ...
  for (const business of inventory.businesses) {
    if (!business.slug || !/^[a-z0-9][a-z0-9-]*$/.test(business.slug)) {
      throw new Error(`Invalid business slug: ${business.slug}`);
    }
    if (!business.title || !business.blueprint_html) {
      throw new Error(`Business ${business.slug} is missing title or blueprint_html`);
    }

    if (slugs.has(business.slug)) {
      throw new Error(`Duplicate normalized slug: ${business.slug}`);
    }
    slugs.set(business.slug, business.blueprint_html);

    const normalizedTitle = normalizeTitle(business.title);
    if (titles.has(normalizedTitle)) {
      throw new Error(`Duplicate normalized title: ${business.title}`);
    }
    titles.set(normalizedTitle, business.slug);
  }

  const declared = inventory.counts?.total_businesses;
  if (Number.isInteger(declared) && declared !== inventory.businesses.length) {
    throw new Error(`Inventory count mismatch: declared ${declared}, found ${inventory.businesses.length}`);
  }
}
...
function classifyBusiness(business) {
  ...
  const cohortOrder = ['clinical', 'financial', 'employment', 'housing', 'environmental', 'safety', 'privacy', 'government', 'legal'];
  for (const item of cohortOrder) {
    if (sensitivity.has(item)) {
      cohort = item;
      break;
    }
  }
  ...
  return {
    primary_archetype: 'authority',
    secondary_archetypes: [...secondaryArchetypes].filter((item) => item !== 'authority'),
    sensitivity: [...sensitivity],
    ymyl_level: highRisk ? 'high' : moderateRisk ? 'moderate' : 'none',
    geographic_model: geographicModel,
    business_model: [...businessModel],
    review_intensity: highRisk ? 'qualified_domain_review' : moderateRisk ? 'enhanced' : 'standard',
    cohort,
...
function unassignedOwner() {
  return { name: null, role: null, contact: null, status: 'unassigned' };
}

function requirementProfile(requirements) {
  return { status: 'planned', requirements, owners: [], evidence: [] };
}

const EPICS = [
  ['epic-01', 'Discovery, sources, and differentiation'],
  ['epic-02', 'Audience, offer, and commercial validation'],
  ['epic-03', 'Brand and design-system configuration'],
  ['epic-04', 'Content and entity models'],
  ['epic-05', 'Information architecture and URL plan'],
  ['epic-06', 'Authority content and trust pages'],
  ['epic-07', 'Product, service, or workflow experience'],
  ['epic-08', 'Original utility and lead magnets'],
  ['epic-09', 'Editorial and domain-review operations'],
  ['epic-10', 'Technical SEO, structured data, and internal linking'],
  ['epic-11', 'Lead capture, newsletter, CRM, and lifecycle'],
  ['epic-12', 'Analytics, attribution, experimentation, and dashboards'],
  ['epic-13', 'Accessibility, performance, security, privacy, and legal review'],
  ['epic-14', 'Off-site authority, partnerships, reviews, and reputation'],
  ['epic-15', 'Migration, launch, indexation, monitoring, and rollback'],
  ['epic-16', 'Continuous content, regulatory, product, and technical maintenance']
];
...
function buildPlan(business, classification, hash, sourceRepository) {
  ...
  return {
    ...
    plan_id: `ainbis-plan:${business.slug}`,
    plan_version: '0.1.0',
    business: {
      slug: business.slug,
      ...
      blueprint_path: business.blueprint_html,
      blueprint_sha256: hash,
      source: business.source ?? null,
      on_main: Boolean(business.on_main),
      developed: Boolean(business.developed),
      launched: Boolean(business.launched)
    },
    state: {
      planning: 'scaffolded',
      indexability: business.launched ? 'public_noindex' : 'planning',
      freshness: hash ? 'current' : 'unknown',
      blockers: [
        'Complete current primary-source and search-landscape research',
        'Validate independent purpose and portfolio differentiation',
        'Assign accountable owners and required reviewers',
        'Specify original utility, launch content graph, conversion, and operating model',
        'Pass all authority and indexability gates'
      ]
    },
    owners: {
      business: unassignedOwner(),
      editorial: unassignedOwner(),
      technical: unassignedOwner(),
      domain_reviewer: classification.review_intensity === 'standard' ? null : unassignedOwner(),
      privacy_security: unassignedOwner(),
      approver: unassignedOwner()
    },
    classification,
    strategy: {
      independent_purpose: { status: 'unassessed', text: '', evidence: [] },
      audiences: [],
      jobs_to_be_done: [],
      problem: { status: 'unassessed', text: '', evidence: [] },
      offer: { status: 'unassessed', text: '', evidence: [] },
      differentiation: { status: 'unassessed', text: '', evidence: [] },
      boundaries: [],
      commercial_model: [],
      overlap_assessment: {
        status: 'unassessed',
    ...
    authority: {
      thesis: { status: 'unassessed', text: '', evidence: [] },
      original_value: [],
      expertise: [],
      first_hand_evidence: [],
      methodology: [],
      maintenance_model: [],
      who_how_why: { who: '', how: '', why: '' }
    },
    research: {
      status: 'not_started',
      source_manifest: 'research-manifest.json',
      primary_sources: [],
      secondary_sources: [],
      open_questions: [],
      effective_date_dependencies: []
    },
    search: {
      topic_boundary: '',
      intent_clusters: [],
      ...
      query_page_map: [],
      collision_status: 'unassessed',
      competitor_and_gap_notes: []
    },
    site_architecture: {
      navigation_model: [],
      pages: [],
      url_policy: [],
      internal_linking_model: [],
      excluded_page_patterns: [
        'Token-swapped location, role, industry, regulation, or keyword pages',
        'Pages without unique purpose, evidence, ownership, maintenance, and indexability decisions'
      ]
    },
    content: {
      launch_portfolio: [],
      expansion_backlog: [],
      editorial_workflow: [
        'research', 'brief', 'draft', 'source verification', 'domain review when required',
        'editorial approval', 'technical and accessibility validation', 'publish noindex',
        'indexability approval', 'monitor', 'refresh or retire'
      ],
      review_policy: [],
      prohibited_content: [
        'Unsupported consequential claims',
        'Fabricated proof, experts, credentials, results, clients, reviews, or statistics',
        'Scaled or doorway content created primarily for search rankings',
        'Superficial rewriting without substantial original value'
      ],
      newsletter_pillars: []
    },
    utilities: [{
      ...
      name: utilityName,
      type: utilityType,
      purpose: 'Provide defensible value beyond informational content',
      status: 'idea',
      inputs: [],
      outputs: [],
      source_basis: [],
      risks: [],
      reviewers: [],
      conversion_role: null
    }],
    conversion: {
      primary_action: 'To be determined from business model and audience intent',
      secondary_actions: [],
      lead_magnets: [],
      forms: [],
      newsletter: 'undecided',
      crm_lifecycle: [],
      ethical_constraints: [
        'No fake urgency, deceptive scarcity, confirm-shaming, hidden fees, or preselected consent',
    ...
    trust: {
      ...
      claim_policy: [],
      proof_policy: [
        'Only genuine, permissioned proof may be presented as real',
        'Pre-launch proof slots must be visibly labeled placeholder or illustrative'
      ],
      disclosures: [],
      corrections: []
    },
    technical: {
      rendering: 'undecided',
      canonical_policy: [],
      robots_policy: ['Generated and review environments remain noindex'],
      sitemap_profiles: [],
      redirect_policy: [],
      structured_data: [],
      localization: [],
    ...
    implementation: {
      ...
      dependencies: [],
      migration: business.launched ? ['Inventory and preserve existing URLs, search equity, analytics, leads, and conversion behavior'] : [],
      launch: [],
      operations: []
    },
    gates: GATES.map(([id, name]) => ({
      id,
      name,
      status: 'not_assessed',
      blocking: true,
    ...
    provenance: {
      generated_at: now,
      generator: GENERATOR,
      inputs: [business.blueprint_html],
      human_review: [],
      change_log: ['Initial deterministic scaffold from authoritative inventory']
    }
  };
}
...
function refreshPlan(existing, business, classification, hash, sourceRepository) {
  ...
  return {
    ...
    state: {
      ...existing.state,
      freshness: changed ? 'stale' : hash ? 'current' : existing.state?.freshness ?? 'unknown',
      blockers: changed
        ? [...new Set([...(existing.state?.blockers ?? []), 'Blueprint changed after the last reviewed plan; review and refresh required'])]
        : existing.state?.blockers ?? []
    },
    provenance: {
      ...existing.provenance,
      generated_at: now,
      generator: GENERATOR,
      inputs: [...new Set([...(existing.provenance?.inputs ?? []), business.blueprint_html])],
      change_log: [
        ...(existing.provenance?.change_log ?? []),
        changed ? 'Refreshed inventory metadata and marked plan stale after blueprint hash change' : 'Refreshed inventory metadata'
      ]
    }
  };
}
...
async function listPlanFiles(root) {
  ...
  async function walk(directory) {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      else if (entry.name === 'plan.json') files.push(entryPath);
    }
  }
...
async function validateParity(inventory, outputRoot) {
  ...
  const planFiles = await listPlanFiles(outputRoot);
  const actual = new Map();
  const errors = [];

  for (const planFile of planFiles) {
    const plan = await loadJson(planFile);
    const slug = plan.business?.slug;
    if (!slug) {
      errors.push(`${planFile}: missing business.slug`);
      continue;
    }
    if (actual.has(slug)) errors.push(`Duplicate plan for ${slug}: ${actual.get(slug)} and ${planFile}`);
    actual.set(slug, planFile);
    if (plan.plan_id !== `ainbis-plan:${slug}`) errors.push(`${planFile}: plan_id does not match slug`);
    if (plan.schema_version !== '1.0.0') errors.push(`${planFile}: unsupported schema_version ${plan.schema_version}`);
  }

  for (const slug of expected) {
    if (!actual.has(slug)) errors.push(`Missing plan for inventory business ${slug}`);
  }
  for (const [slug, planFile] of actual) {
    if (!expected.has(slug)) errors.push(`Orphan plan not found in inventory: ${slug} (${planFile})`);
  }

  if (errors.length > 0) {
    throw new Error(`Portfolio plan validation failed:\n- ${errors.join('\n- ')}`);
  }
...
async function main() {
  ...
  const blueprintsRoot = args.blueprintsRoot ? path.resolve(args.blueprintsRoot) : null;
  const inventory = await loadJson(inventoryPath);
  validateInventory(inventory);

  if (args.mode === 'validate') {
    const result = await validateParity(inventory, outputRoot);
    console.log(`Validated ${result.actual} business authority plans against ${result.expected} inventory records.`);
    return;
  }
  ...
  for (const business of inventory.businesses) {
    ...
    const hash = await blueprintHash(business, blueprintsRoot);
    const directory = path.join(outputRoot, business.slug[0], business.slug);
    ...
    if (exists) {
      ...
      if (args.mode === 'refresh') {
        plan = refreshPlan(existing, business, classification, hash, args.sourceRepository);
      ...
      plan = buildPlan(business, classification, hash, args.sourceRepository);
      await writeJson(planPath, plan);
      await writeFile(markdownPath, renderMarkdown(plan), 'utf8');
      created += 1;
    }
    ...
    indexEntries.push({
      ...
      developed: Boolean(business.developed),
      launched: Boolean(business.launched),
      plan_path: path.relative(process.cwd(), planPath).replaceAll(path.sep, '/'),
      planning_state: plan.state?.planning ?? 'unknown',
      indexability: plan.state?.indexability ?? 'unknown',
      freshness: plan.state?.freshness ?? 'unknown',
      primary_archetype: plan.classification?.primary_archetype ?? 'unknown',
      cohort: plan.classification?.cohort ?? null,
      review_intensity: plan.classification?.review_intensity ?? 'unknown'
    });
  }

  indexEntries.sort((left, right) => left.slug.localeCompare(right.slug));
  const registry = {
    schema_version: '1.0.0',
    generated_at: new Date().toISOString(),
    generator: GENERATOR,
    source_inventory: path.relative(process.cwd(), inventoryPath).replaceAll(path.sep, '/'),
    counts: {
      businesses: indexEntries.length,
      developed: indexEntries.filter((entry) => entry.developed).length,
      launched: indexEntries.filter((entry) => entry.launched).length,
      created,
      refreshed,
      preserved
    },
    businesses: indexEntries
  };

  await writeJson(path.join(outputRoot, 'index.json'), registry);
  await writeJson(path.join(outputRoot, 'summary.json'), {
    schema_version: registry.schema_version,
    generated_at: registry.generated_at,
    counts: registry.counts,
    planning_states: Object.groupBy(indexEntries, (entry) => entry.planning_state),
    indexability_states: Object.groupBy(indexEntries, (entry) => entry.indexability),
    cohorts: Object.groupBy(indexEntries, (entry) => entry.cohort ?? 'unassigned')
  });

  const validation = await validateParity(inventory, outputRoot);
  console.log(`Plans ready: ${validation.actual}. Created ${created}, refreshed ${refreshed}, preserved ${preserved}.`);
}
